...
func UpdateItem(path string, table string, item models.Item) {
...
func DeleteItem(path string, table string, id int) {
	sqlStatement := `DELETE FROM '%s' WHERE ID = '%d';`
	sqlStatement = fmt.Sprintf(sqlStatement, quote_str(table), id)
	db_exec(path, sqlStatement)
}

func DeleteTable(path string, table string, id int) {
	sqlStatement := `DELETE FROM '%s' WHERE ID = '%d';`
...
func RenameTable(path string, oldName, newName string, id int) {
